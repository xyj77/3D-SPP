...
model = Sequential()
model.add(SPP3D(pooling_regions, input_shape=input_shape))
model.summary()
...
for img_size in [8, 16]:
    ...
    if dim_ordering == 'th':
        X = np.random.rand(batch_size, num_channels, img_size, img_size*2, img_size)
        row_length = [float(X.shape[2]) / i for i in pooling_regions]
        col_length = [float(X.shape[3]) / i for i in pooling_regions]
        hight_length = [float(X.shape[4]) / i for i in pooling_regions]
    elif dim_ordering == 'tf':
        X = np.random.rand(batch_size, img_size, img_size*2, img_size, num_channels)
        # print X.shape
        row_length = [float(X.shape[1]) / i for i in pooling_regions]
        col_length = [float(X.shape[2]) / i for i in pooling_regions]
        hight_length = [float(X.shape[3]) / i for i in pooling_regions]
                ...
                for jy in range(num_pool_regions):
                    for kz in range(num_pool_regions):
                        for cn in range(num_channels):
                            ...
                            z1 = int(round(kz * hight_length[pool_num]))
                            z2 = int(round(kz * hight_length[pool_num] + hight_length[pool_num]))

                            if dim_ordering == 'th':
                                m_val = np.max(X[batch_num, cn, x1:x2, y1:y2, z1:z2])
                            elif dim_ordering == 'tf':
                                m_val = np.max(X[batch_num, x1:x2, y1:y2, z1:z2, cn])

                            np.testing.assert_almost_equal(
                               m_val, Y[batch_num, idx], decimal=6)
                            idx += 1

print('3D Spatial pyramid pooling test passed')
